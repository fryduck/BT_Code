/*************************************************************************
	...
	> Author: ma6174
	> Mail: ma6174@163.com 
	> Created Time: 2015年11月02日 星期一 17时41分34秒
 ************************************************************************/
...
typedef struct _Bitmap{
	...
	int bitfield_length;		//保存位图所占的总字节数
...
void release_memory_in_field();										//释放bitfield.c中动态分配的内存
int print_bitfield(Bitmap *bitmap);									//打印位图值，用于调试
int restore_bitmap();												//将位图存储到文件中
																	//在下次下载时，先读取该文件获取已经下载的速度
int is_interested(Bitmap *dst,Bitmap *src);							//拥有位图的src的peer是否对拥有dst位图的peer感兴趣

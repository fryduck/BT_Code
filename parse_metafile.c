/*************************************************************************
	...
	> Author: ma6174
	> Mail: ma6174@163.com 
	> Created Time: 2015年10月28日 星期三 14时45分20秒
 ************************************************************************/
...
unsighed char	info_hash[20];
unsighed char	peer_id[20];
...
int read_announce_list()
{
	...
	if(find_keyword("13:announce-list",&i)==0){
		...
		while(metafile_content[i]!='e'){
			...
			//只处理以http开头的tracker地址，不出里以udp开头的地址
			if(memcmp(&metafile_content[i],"http",4)==0){
				...
				if(announce_list_head==NULL)
					annnounce_list_head=node;
				else{
					p=announce_list_head;
					while(p->next!=NULL)	p=p->next;//使p指针指向最后一个节点
...
int add_an_announce(char *url)
{
	...
	p=announce_list_head;
	if(p==NULL)	{annouce_list_head=q; return -1;}
...
int get_info_hash()
{
	...
	SHAl_CTX context;
	SHAlInit(&context);
	SHAlUpdate(&context,&metafile_content[begin],end-begin+1);
	SHAlFinal(info_hahs,&context);
...
void release_memory_in_parse_metafile()
{
	...
	while(announce_list_head!=NULL){
		p=announce_list_head;
		announce_list_head=announce_list-head->next;
		free(p);
	}
...
int parse_metafile(char *metafile)
{
	...
	if(ret<0) {printf("%s:%d wrong",__FILE__,__LINE__);return -1;}
	ret=get_peer_id();
	if(ret<0) {printf("%sL%d wrong",__FILE__,__LINE__);return -1;}

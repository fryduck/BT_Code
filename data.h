/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2016年01月24日 星期日 20时21分45秒
 ************************************************************************/
...
typedef struct _Btcache{
    ...
    unsigned char read_write;   // 是发送给peer的数据还是接收到的数据
                                   // 若数据从硬盘读出，read_write位为0
                                   // 若数据将要写入硬盘，read_write位为1
    unsigned char is_full;      // 缓冲区是否满
    unsigned char is_writed;    // 缓冲区中的数据是否写入到硬盘中

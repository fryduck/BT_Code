/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: Thu 05 May 2016 12:51:43 AM JST
 ************************************************************************/
...
int main(int argc, char *argv[])
{
    ...
    ret = parse_metafile(argv[i]);  // 解析种子文件
    if (ret != 0)   { printf("%s:%d error\n", __FILE__, __LINE__); return -1; }

    ret = create_files();   // 创建用于保存下载数据的文件
    if (ret != 0)   { printf("%s:%d error\n", __FILE__, __LINE__); return -1; }

    ret = create_bitfiled();    // 创建位图
    if (ret != 0)   { printf("%s:%d error\n", __FILE__, __LINE__); return -1; }
    
    ret = create_btcache();    // 创建缓冲区
    if (ret != 0)   { printf("%s:%d error\n", __FILE__, __LINE__); return -1; }

    init_unchoke_peers();   // 初始化非阻塞peer

    download_upload_with_peers();   // 负责与所有Peer收发数据，交换消息

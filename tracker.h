/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: Tue 03 May 2016 06:01:07 PM JST
 ************************************************************************/
...
// 获取Tracker返回的消息类型
int get_reponse_type(char *buffer, int len, int *total_length);
// 解析第一种Tracker返回的消息
int parse_tracker_reponse1(char *buffer, int ret, char *redirection, int len);
// 解析第二种Tracker返回的消息
int parse_tracker_reponse2(char *buffer, int ret);
// 为已建立连接的peer创建peer节点并加入到peer链表中
